...
3. Kiosk: `cd web && npm install && npm run dev` (:9002) — unit test `npm test`, E2E `npm run e2e`
...
4. `dotnet run --project server/src/ZyrexMES.Api`  ? http://localhost:9090/swagger
...
- **Port 9090 dipakai proses lain** ? matikan proses pemilik port atau ubah `applicationUrl` di `server/src/ZyrexMES.Api/Properties/launchSettings.json`.
...
- Health check: `GET http://localhost:9090/health` ? `200 {"status":"ok"}`
...
```bash
export MES_LEGACY__URL="http://192.168.1.245:9090/API/TE/PostData"
...
```bash
TOKEN=$(curl -s -X POST localhost:9090/api/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"username":"admin","password":"..."}' | jq -r .token)

# 1) master data dari tabel staging (idempoten, kode LGCY-*, Source=Legacy)
curl -X POST localhost:9090/api/migration/import-master -H "Authorization: Bearer $TOKEN"

# 2) transaksi 12 bulan dari staging (window maks 366 hari per panggilan)
curl -X POST localhost:9090/api/migration/import-transactions \
...
curl localhost:9090/api/migration/reconciliation/report -H "Authorization: Bearer $TOKEN"
curl -X POST localhost:9090/api/migration/reconciliation/sample \
...
```bash
curl -X POST localhost:9090/api/production/scan \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
...
curl -X POST localhost:9090/api/quality/results \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
